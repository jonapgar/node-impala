  ...
  _createClass(ImpalaClient, [{
    ...
    value: function query(sql, callback) {
      ...
      if (!client || !connection) {
        ...
        handlePromise = client.query(query).then(handle=>{
        ...
        handlePromise.then(async function (handle) {
          
          let state = client.get_state(handle)
          let metadata = client.get_results_metadata(handle)

          let results = []

          while (true) {
            let result = await client.fetch(handle,false,8192)
            results.push(result)
            if (result.has_more) 
                continue
            break
          ...
          return [handle, await state, results,await metadata];

        }).then(function ([handle, state, results, metaData]) {
          ...
          var processedData = ImpalaClient.processData(results, schemas, resultType);
      ...
      deferred.promise.nodeify(callback);
      deferred.promise.handle = handlePromise
      return deferred.promise;
    }
  }]);
 ...
 */
ImpalaClient.processData = function (results, schemas, type) {
  switch (type) {
    case 'map':
      {
        var resultArray = [];
         ...
         let data = results[j].data
          ...
          for (var _i2 = 0; _i2 < data.length; _i2 += 1) {
            ...
            for (var _j = 0; _j < schemaNames.length; _j += 1) {
              ...
              let value = data[_i2].split('\t')[_j]
              let dataType = schemas[_j].type.toLowerCase()
              switch (dataType) {
              ...
              resultObject[schemaNames[_j]] = value;
            }
    ...
    case 'boolean':
      {
        return results !== undefined && schemas !== undefined;
      }
    default:
      {
